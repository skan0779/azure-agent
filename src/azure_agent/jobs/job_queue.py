    ...
    *,
    ...
    if idempotency_key:
            ...
            if existing_job is None:
                ...
                if existing_job.get("enqueue_state") == "enqueue_failed":
                    reenqueue_fields: dict[str, str] = {
                        ...
                        "thread_id": str(existing_job.get("thread_id", thread_id)),
                        "user_id": str(existing_job.get("user_id", user_id)),
                        "created_at": str(existing_job.get("created_at", created_at)),
                    }
                    if idempotency_key:
                    ...
                    try:
                        ...
                        await redis_client.hset(
                            ...
                            mapping={
                                ...
                                "enqueue_failed_at": datetime.now(
                                    timezone.utc
                                ).isoformat(),
                            },
                        )
        ...
        if not claimed:
            ...
            raise RuntimeError(
                "[jobs.job_queue] Failed to resolve idempotency winner."
            )
...
async def cancel_job(redis_client: RedisCluster, job_id: str) -> bool:
    """
